...
const getEmotionByPublicationId = (
  emotions: PopoverEmotions[],
  publicationId: string,
): PopoverEmotions | undefined => {
...
export const MeetingRoom = ({ params }) => {
  ...
  const [memberTranscript, setMemberTranscript] = useState<{
    member: string
    data: string
  }>()
  ...
  const onJoinChannel = useCallback(async () => {
    ...
    if (swCxt) {
      ...
      const myAudioqInputStream = new LocalAudioStream(
      ...
      const subscribeAndAttach = async (publication: RoomPublication) => {
        ...
        switch (stream.contentType) {
          case 'video':
            const videoMedia: HTMLVideoElement = document.createElement('video')
            ...
            break
          case 'audio':
            const audioMedia: HTMLAudioElement = document.createElement('audio')
            ...
            <GridItem area={'heading'}>
              ...
              >
                <GridItem area={'headingItem'}>
                  {memberTranscript != undefined ? (
                    <p>
                      {memberTranscript.member}:{memberTranscript.data}
                    </p>
                  ) : (
                    <></>
                  )}
                ...
                <div className="videoBox">
                  <div id="remoteMediaArea">
                    {Object.keys(userAreaDocuments).map((publicationId) => {
                      ...
                      const sortedEmotion = getEmotionByPublicationId(
                        emotion,
                        publicationId,
                      )
                        ...
                        >
                          ...
                          <div>
                            {remoteUserArea && (
                              <div
                                ref={(node) =>
                                  node && node.appendChild(remoteUserArea)
                                }
                              />
                            )}
                          </div>
